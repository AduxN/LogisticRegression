...
data = data.map(lambda x: grade_mapping[x] if x in grade_mapping else np.nan).dropna(subset=['Algoritmy a údajové štruktúry 2'])


subjects_to_keep = ['Algoritmy a údajove štruktúry 1', 'Informatika 3', 'Informatika 2', 'Algoritmy a údajové štruktúry 2']
data = data[subjects_to_keep]
aus2 = data['Algoritmy a údajové štruktúry 2']

# Define the target variable (pass/fail)
data['pass_fail'] = data.apply(lambda row: 1 if row['Algoritmy a údajové štruktúry 2'] > 0 else 0, axis=1)


# Split the data into features and target
X = data.drop(columns=['Algoritmy a údajové štruktúry 2', 'pass_fail'])
y = data['pass_fail']
print(data)
